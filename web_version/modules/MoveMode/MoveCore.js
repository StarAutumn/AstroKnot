...
import { clearSelected, setSelectedNode, getPrimarySelectedId, updateSelectionUI, deleteSelectedNodes, showToast, addNode, renamePrimaryNode, toggleChildren, completeAddConnection, completeRemoveConnection, cancelConnectionMode } from '../module5_SelectAndEdit.js';
import { hideBlankContextMenu, showBlankContextMenu, showContextMenu, hideContextMenu } from '../module8_ContextMenu.js';
 ...
 * 创建节点并添加到项目和场景
 */
function createNodeInProject({ name, desc, sizeScale, nodeType, blockType, isStepFlow, parentId, offsetX, offsetY, asNextStep }) {
 ...
 * 计算子节点的序号名称
 */
function getNextChildName(parentNode, pattern, prefix, startIndex) {
...
export function initMoveMode() {
  ...
  if (addRootBtn) {
    addRootBtn.addEventListener('click', () => createNodeInProject({ name: '新根节点', desc: '📖 顶层节点', sizeScale: 2.0 }));
  }

  const addStepRootBtn = document.getElementById('addStepRootNodeBtn');
  if (addStepRootBtn) {
    addStepRootBtn.addEventListener('click', () => createNodeInProject({ name: '下一步1', desc: '📖 步骤节点', sizeScale: 2.0, isStepFlow: true }));
  }

  const addBlockRootBtn = document.getElementById('addBlockRootNodeBtn');
  if (addBlockRootBtn) {
    addBlockRootBtn.addEventListener('click', () => createNodeInProject({ name: '新块节点', desc: '📦 块编辑器节点', sizeScale: 2.0, nodeType: 'block' }));
  }

  const addVideoRootBtn = document.getElementById('addVideoRootNodeBtn');
  if (addVideoRootBtn) {
    addVideoRootBtn.addEventListener('click', () => createNodeInProject({ name: '新视频块节点', desc: '🎬 视频播放器节点', sizeScale: 2.0, nodeType: 'block', blockType: 'video' }));
  }

  const addBlockStepRootBtn = document.getElementById('addBlockStepRootNodeBtn');
  if (addBlockStepRootBtn) {
    addBlockStepRootBtn.addEventListener('click', () => createNodeInProject({ name: '块步骤 1', desc: '📦 块编辑器步骤节点', sizeScale: 2.0, nodeType: 'block', isStepFlow: true }));
  }
