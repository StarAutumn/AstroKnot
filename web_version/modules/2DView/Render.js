...
let _lineItemsMapArrayRef = null;  // 用数组引用检测变化，而非 length
...
export function draw() {
  ...
  _rebuildCrossEdgeIndex();       // 跨层连线节点索引（数组引用变化时重建）
  _rebuildAnimationsIndex();      // 动画索引（数组引用变化时重建）

  // 预索引 lineItems（仅当数组引用变化时重建，避免 rebuildAllLines 后缓存失效）
  if (_lineItemsMapArrayRef !== appState.lineItems) {
    _lineItemsMap.clear();
    for (const item of appState.lineItems) {
      _lineItemsMap.set(`${item.startId}->${item.endId}`, item);
    }
    _lineItemsMapArrayRef = appState.lineItems;
  }
  ...
  for (const [id, pos] of positionMap.entries()) {
    ...
    if (node) {
      ...
      areas.push({ id, x: pos.x, y: pos.y, width: BASE_NODE_WIDTH * scale, height: BASE_NODE_HEIGHT * scale });
  ...
  // 仅在有动画/自由绘制/折叠变化时自刷新；常规帧由 3D 动画循环驱动 refresh2DView
  if (visible && (animations.length > 0 || isFreeDrawing)) {
