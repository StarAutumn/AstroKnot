...
import { saveSettingsToStorage } from './UI/Theme.js';
...
function applyLoadedData(data, folderName, folderPath) {
  ...
  appState.currentProjectSavePath = folderPath;
...
export async function saveNetworkToFile() {
  ...
  try {
    ...
    if (result.success) {
      // 存储父目录（不含项目名），不同项目共享同一父目录
      const sep = result.path.includes('\\') ? '\\' : '/';
      const parts = result.path.replace(/[\\/]$/, '').split(/[\\/]/);
      parts.pop();
      appState.currentProjectSavePath = parts.join(sep);
...
export async function saveAllProjects() {
  ...
  if (parentPath) {
    appState.currentProjectSavePath = parentPath;
    saveSettingsToStorage();
  }
