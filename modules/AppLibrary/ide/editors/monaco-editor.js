...
import { ensureMonaco, toMonacoLang } from '../../../richEditor/core/code-blocks.js';
...
export class SandboxMonacoEditor {
  ...
  dispose() {
    // 销毁所有 models（try-catch 防止个别 model dispose 异常阻断后续清理）
    for (const [, entry] of this._models) {
      try { entry.model.dispose(); } catch (_) { /* Canceled error is expected */ }
    }
    this._models.clear();

    // 销毁编辑器实例（Monaco dispose 可能抛出 Canceled 异步错误，由全局 handler 捕获）
    if (this._editor) {
      try { this._editor.dispose(); } catch (_) { /* Canceled error is expected */ }
      this._editor = null;
    }
