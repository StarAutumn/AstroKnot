...
function getContextMenuItems(imgData) {
  ...
  if (type === 'image') {
    ...
    items = [
      { label: '━━ 幻灯片编辑 ━━', action: null, separator: true },
      { label: '🎬 打开编辑器', action: function () { openSlideshowEditor(imgData); } },
      { label: '━━ 播放设置 ━━', action: null, separator: true },
      { label: imgData.autoplay ? '✓ 自动播放' : '○ 自动播放', action: function () { imgData.autoplay = !imgData.autoplay; renderAll(); } },
      { label: imgData.loop !== false ? '✓ 循环播放' : '○ 循环播放', action: function () { imgData.loop = imgData.loop === false; renderAll(); } },
      { label: imgData.showNavigation !== false ? '✓ 箭头导航' : '○ 箭头导航', action: function () { imgData.showNavigation = imgData.showNavigation === false; renderAll(); } },
      { label: imgData.showPagination !== false ? '✓ 分页圆点' : '○ 分页圆点', action: function () { imgData.showPagination = imgData.showPagination === false; renderAll(); } },
      { label: '━━ 切换效果 ━━', action: null, separator: true },
      { label: (imgData.effect === 'slide' ? '● ' : '○ ') + '滑动', action: function () { imgData.effect = 'slide'; renderAll(); } },
      { label: (imgData.effect === 'fade' ? '● ' : '○ ') + '淡入', action: function () { imgData.effect = 'fade'; renderAll(); } },
      { label: (imgData.effect === 'cube' ? '● ' : '○ ') + '立方体', action: function () { imgData.effect = 'cube'; renderAll(); } },
      { label: (imgData.effect === 'coverflow' ? '● ' : '○ ') + '3D流', action: function () { imgData.effect = 'coverflow'; renderAll(); } },
      { label: (imgData.effect === 'flip' ? '● ' : '○ ') + '翻页', action: function () { imgData.effect = 'flip'; renderAll(); } },
      { label: (imgData.effect === 'cards' ? '● ' : '○ ') + '卡片', action: function () { imgData.effect = 'cards'; renderAll(); } },
      { label: '━━ 图层顺序 ━━', action: null, separator: true }
    ].concat(layerItems).concat([
      { label: '━━ 删除 ━━', action: null, separator: true },
      deleteItem
