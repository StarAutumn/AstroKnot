...
export class GuideOverlay {
  constructor() {
    ...
    this._bubblePosition = 'left';  // 'left' | 'right'
    this._visible = false;
  ...
  /** 设置气泡位置（left=默认左侧，right=右侧，避免被菜单遮挡） */
  setBubblePosition(position) {
    this._bubblePosition = position || 'left';
  }
  ...
  _updateHighlight() {
    ...
    // 高亮区域留 8px 内边距
    const pad = 8;
    // getBoundingClientRect 返回视口坐标，但 SVG 在 overlay 内部，
    // overlay 有 top: titlebar-height 偏移，需要减去该偏移使 SVG 坐标对齐
    const overlayOffsetY = this.overlay ? this.overlay.offsetTop : 0;
    this._rect = {
      x: rect.left - pad,
      y: rect.top - pad - overlayOffsetY,
      w: rect.width + pad * 2,
      h: rect.height + pad * 2,
      r: 10,
    };
  ...
  /** 绘制 SVG 箭头连线：气泡边缘 → 高亮切窗中心（根据气泡位置自动选择起点） */
  _updateArrowLine(targetRect) {
    // 移除旧箭头
    if (this._arrowLine) { this._arrowLine.remove(); this._arrowLine = null; }
    if (!targetRect || !this.svgMask) return;

    // 使用 getBoundingClientRect 获取气泡在视口中的实际像素位置
    const bubbleRect = this.bubble.getBoundingClientRect();
    // 气泡 Y 坐标需减去 overlay 偏移，转为 SVG 内部坐标系
    const overlayOffsetY = this.overlay ? this.overlay.offsetTop : 0;
    const bubbleCenterY = bubbleRect.top + bubbleRect.height / 2 - overlayOffsetY;

    // 根据气泡位置决定箭头起点：右侧模式→从气泡左边缘出发；左侧模式→从气泡右边缘出发
    let bubbleEdgeX;
    if (this._bubblePosition === 'right') {
      bubbleEdgeX = bubbleRect.left;  // 气泡左边缘
    } else {
      bubbleEdgeX = bubbleRect.right;  // 气泡右边缘
    }

    // 目标切窗中心（已在 _updateHighlight 中转为 SVG 坐标）
    const targetX = targetRect.x + targetRect.w / 2;
    const targetY = targetRect.y + targetRect.h / 2;

    // 绘制曲线：用二次贝塞尔，控制点偏移
    const cpx = bubbleEdgeX + (targetX - bubbleEdgeX) * 0.4;
    const cpy = bubbleCenterY;
    const pathD = `M ${bubbleEdgeX} ${bubbleCenterY} Q ${cpx} ${cpy}, ${targetX} ${targetY}`;
    ...
    // 目标端箭头三角
    const arrowSize = 8;
    const angle = Math.atan2(targetY - bubbleCenterY, targetX - bubbleEdgeX);
