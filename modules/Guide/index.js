...
export function jumpToStep(stepIndex) {
  ...
  const ok = sm.goToStep(stepIndex);
  if (ok) {
    _showStep(sm.currentIndex);
    if (overlay && !overlay.overlay.classList.contains('show')) {
      overlay.show();
    }
    console.log(`[Guide] 已跳转到第 ${stepIndex + 1} 步: ${sm.currentStep?.id || '?'}`);
  }
  return ok;
}

// 挂载到 window，方便控制台直接调用测试
if (typeof window !== 'undefined') {
  window.__guideJumpToStep = jumpToStep;
}
...
function _showStep(index) {
  ...
  // 配置气泡位置（默认左侧，部分步骤可指定右侧避免被菜单遮挡）
  overlay.setBubblePosition(step.bubblePosition || 'left');
